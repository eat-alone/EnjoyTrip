...
const getLetterCount = async (userId) => {
  await getReceiveCount(userId, (response) => {
    ...
    >
      <div class="flex sm:flex-auto">
        <router-link :to="{ name: 'home' }" class="-m-1.5 p-1.5">
          ...
          <img
            class="h-10 w-auto"
            src="https://cdn-icons-png.flaticon.com/128/4652/4652340.png"
            alt=""
          />
        </router-link>
      ...
      <PopoverGroup class="hidden lg:flex lg:gap-x-12">
        ...
        <router-link
          :to="{ name: 'user-search' }"
          class="text-sm font-semibold mr-20 leading-6 text-gray-900"
          >유저 검색
        </router-link>
      </PopoverGroup>
      ...
      <div v-if="isLogin" class="hidden lg:flex">
        ...
        <div class="relative">
          <router-link :to="{ name: 'letter-receive' }" class="-m-1.5 p-1.5">
            <img
              class="h-8 w-auto"
              src="https://cdn-icons-png.flaticon.com/128/12328/12328821.png"
              alt=""
            />
          </router-link>
          <p
            v-if="noReadLetterCountvalue"
        ...
        <router-link
          ...
          class="text-sm font-semibold leading-6 text-gray-900 ml-5 mt-4"
          >마이페이지 <span aria-hidden="true">&rarr;</span></router-link
        >
        <router-link
          to="/"
          @click.prevent="logout"
          class="text-sm font-semibold ml-3 leading-6 text-gray-900 mt-4"
          >로그아웃 <span aria-hidden="true">&rarr;</span></router-link
        >
        <p class="text-sm font-semibold ml-5 leading-6 text-gray-900 mt-4">
